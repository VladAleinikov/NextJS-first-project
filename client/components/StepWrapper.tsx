...
const StepWrapper: React.FC<StepWrapperProps> = ({ activeStep, steps }) => {
  return (
    <ol className="flex items-center w-full max-w-[700px] mb-4 text-sm font-medium text-center text-gray-500 dark:text-gray-400 sm:text-base">
      <li className="flex md:w-full items-center text-blue-600 dark:text-blue-500 sm:after:content-[''] after:w-full after:h-1 after:border-b after:border-gray-200 after:border-1 after:hidden sm:after:inline-block after:mx-6 xl:after:mx-10 dark:after:border-gray-700">
        <span className="flex items-center after:content-['/'] sm:after:hidden after:mx-2 after:text-gray-200 dark:after:text-gray-500">
          ...
          <span className="text-nowrap">{steps[0]}</span>
          {/* Данные <span className="hidden sm:inline-flex sm:ms-2">трека</span> */}
        </span>
      </li>
      {steps
        .filter((e, id) => id !== 0 && id !== steps.length - 1)
        .map((step, id) => (
          <li
            key={step}
            className={
          ...
          >
            <span className="flex items-center after:content-['/'] sm:after:hidden after:mx-2 after:text-gray-200 dark:after:text-gray-500">
              {activeStep >= id + 2 ? (
                ...
                <span className="w-3.5 sm:w-4 me-2.5">{id + 2}</span>
              )}
              <span className="text-nowrap">{steps.at(id + 1)}</span>
            </span>
          </li>
        ))}
      <li
      ...
      >
        <span className="me-2">{steps.length}</span>
        <span className="text-nowrap">{steps.at(-1)}</span>
      </li>
    </ol>
  );
};
